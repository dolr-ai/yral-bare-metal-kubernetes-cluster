...
public enum AuthMachine {
    ...
    public static func transition(
        ...
        case (_, .sessionEstablished(let session)):
            return (commit(session.isAIAccount ? .signedInAsBot(session) : .signedIn(session), snapshot), .none)
    ...
    private static func commit(_ state: State, _ snapshot: Snapshot) -> Snapshot {
        Snapshot(state: state, context: Context(
            properties: resetProperties(onSessionChange: snapshot.state, next: state, from: snapshot.context.properties)
        ))
    }
