...
/// Social + phone sign-in flows for `YralAuthClient` — Kotlin
...
extension YralAuthClient {
    ...
    public func socialAuthorizationURL(provider: YralSocialProvider) throws -> URL {
        let codeVerifier = try YralPKCE.generateCodeVerifier()
        let codeChallenge = YralPKCE.generateCodeChallenge(codeVerifier: codeVerifier)
        ...
        components.host = YralAppConfiguration.oauthBaseURL
        components.path = "/oauth/auth"
        components.queryItems = [
            URLQueryItem(name: "provider", value: provider.wireValue),
            URLQueryItem(name: "client_id", value: YralAuthDataSource.clientID),
            URLQueryItem(name: "response_type", value: "code"),
            URLQueryItem(name: "response_mode", value: provider.responseMode),
            URLQueryItem(
                name: "redirect_uri",
                value: YralAuthDataSource.redirectURI(scheme: redirectScheme)
            ),
        ...
        guard let url = components.url else {
            throw YralAuthError.oauthFailed(errorDescription: "OAuth URL construction failed")
        }
        return url
    }
    ...
    public func handleOAuthCallbackResult(_ result: YralOAuthResult) async throws {
        switch result {
        case let .success(code, state):
            guard state == currentCodeChallenge else {
                currentProvider = nil
                throw YralAuthError.stateMismatch
            }
        ...
        case let .failure(error, errorDescription):
            currentProvider = nil
            throw YralAuthError.oauthFailed(
    ...
    func authenticate(code: String, currentUserPrincipal: String?) async throws {
        guard let codeVerifier = pendingCodeVerifier else {
            throw YralAuthError.oauthFailed(errorDescription: "No in-flight OAuth flow")
        }
