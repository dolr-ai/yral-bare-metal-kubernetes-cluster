...
struct PKCEAndJWTParserTests {
  ...
  func verifierUniqueness() throws {
    let first = try PKCE.generateCodeVerifier()
    let second = try PKCE.generateCodeVerifier()
    #expect(first != second)
  }
  ...
  static func makeJWT(payloadJSON: String) -> String {
    ...
    return
      "\(base64URL(#"{"alg":"ES256","typ":"JWT"}"#)).\(base64URL(payloadJSON)).\(base64URL("signature"))"
  }
  ...
  func audiencePolymorphism() throws {
    ...
    #expect(try JWTParser.parsePayload(of: arrayJWT).audience == ["a", "b"])

    // Absent → empty.
    let absentJWT = Self.makeJWT(
      payloadJSON: #"{"exp":1,"iat":1,"iss":"auth.yral.com","sub":"s"}"#
    )
    #expect(try JWTParser.parsePayload(of: absentJWT).audience.isEmpty)
  }

  @Test("missing required claim throws")
  func missingClaim() {
    let jwt = Self.makeJWT(payloadJSON: #"{"iss":"auth.yral.com"}"#)
    #expect(throws: AuthError.missingRequiredClaim("exp")) {
      _ = try JWTParser.parsePayload(of: jwt)
    }
  }
  ...
  func malformedJWT() {
    #expect(throws: AuthError.malformedJWT(reason: "expected 3 segments, got 2")) {
      _ = try JWTParser.parsePayload(of: "two.segments")
    }
  }
}
