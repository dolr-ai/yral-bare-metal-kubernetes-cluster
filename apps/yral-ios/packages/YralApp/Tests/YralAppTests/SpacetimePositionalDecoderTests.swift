...
struct SpacetimePositionalDecoderTests {
  ...
  func optionBoolField() throws {
    ...
    #expect(
      try SpacetimePositionalDecoder.optionBool(
  ...
  func userProfileSelfView() throws {
    ...
    guard case .mainAccount(let aiAccounts) = profile.accountType else {
  ...
  func userProfileOtherView() throws {
    ...
    guard
      case .pro(let freeVideoCreditsLeft, let totalVideoCreditsAlloted) = profile.subscriptionPlan
    else {
    ...
    guard case .botAccount(let owner) = profile.accountType else {
