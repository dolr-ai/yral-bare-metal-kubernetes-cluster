...
pub fn get_redis_store_ca_cert_pem() -> Result<Vec<u8>, anyhow::Error> {
    Ok(normalize_pem(
...
pub fn get_redis_store_client_cert_pem() -> Result<Vec<u8>, anyhow::Error> {
    Ok(normalize_pem(
...
pub fn get_redis_store_client_key_pem() -> Result<Vec<u8>, anyhow::Error> {
    Ok(normalize_pem(
...
fn get_redis_store_hosts_from_env() -> Vec<String> {
...
pub async fn init_new_dragonfly_redis(
    ...
    let hosts = get_redis_store_hosts_from_env();
...
pub async fn init_dragonfly_redis_for_test() -> Result<Arc<DragonflyPool>, KVError> {
    ...
    let ca_bytes = get_redis_store_ca_cert_pem().expect("Failed to read CA cert");
    let cert_bytes = get_redis_store_client_cert_pem().expect("Failed to read client cert");
    let key_bytes = get_redis_store_client_key_pem().expect("Failed to read client key");
...
impl DragonflyKV {
    pub async fn new() -> Result<Self, anyhow::Error> {
        let ca_cert_bytes = get_redis_store_ca_cert_pem().expect("Failed to read newCA cert");
        let client_cert_bytes =
            get_redis_store_client_cert_pem().expect("Failed to read client cert");
        let client_key_bytes = get_redis_store_client_key_pem().expect("Failed to read client key");
