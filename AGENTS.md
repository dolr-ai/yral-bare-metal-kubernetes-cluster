...
- **Self-hosted DNS (PowerDNS + ExternalDNS):** Authoritative DNS hosted in-cluster on PowerDNS (worker nodes, `kubernetes/infrastructure/external-dns/`). Uses the **LMDB backend** (file-based key-value store on PVC — no external database, supports RFC2136 dynamic updates + TSIG keys). The bind backend does NOT support DNS updates (per PowerDNS docs). ExternalDNS uses `--provider=rfc2136` to sync annotated Services to PowerDNS via TSIG-signed dynamic updates. cert-manager uses RFC2136 DNS-01 (same TSIG key) for wildcard certs. Zone + TSIG key created by init container using `pdnsutil`. Glue records at Namecheap managed via `ansible/roles/namecheap-dns/` (per-host `namecheap_nameserver` annotation in `hosts.yml`). 11 nameservers = Namecheap API max (ICANN allows 13, but Namecheap limits to 11). Phase 1: saikat.dev. Phase 2: yral.com (decommission Cloudflare).
