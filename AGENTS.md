...
- **Self-hosted DNS (PowerDNS + ExternalDNS):** Authoritative DNS hosted in-cluster on PowerDNS (worker nodes, `kubernetes/infrastructure/external-dns/`). Uses the **bind backend** — zone files are text ConfigMaps committed to git (no database, no SQLite/MySQL/CNPG). ExternalDNS uses `--provider=rfc2136` to sync annotated Services to PowerDNS via TSIG-signed dynamic updates. cert-manager uses RFC2136 DNS-01 (same TSIG key) for wildcard certs. Dynamic records (service A records, ACME challenges) live in an ephemeral journal (emptyDir) — auto-recovered by ExternalDNS/cert-manager on pod restart. Glue records at Namecheap managed via `ansible/roles/namecheap-dns/` (per-host `namecheap_nameserver` annotation in `hosts.yml`). 11 nameservers = Namecheap API max (ICANN allows 13, but Namecheap limits to 11). Phase 1: saikat.dev. Phase 2: yral.com (decommission Cloudflare).
