- Snowplow bad events are human readable but the enriched events are not. How come? Depending on the answer, we can decide if we want to keep it this way. I've also noticed I get bad events although bad events should be 0 and enriched should matched the raw count because we should be able to enrich all of them and not lose any events. Where and why are we losing events to the "bad" topic? Can we fix the underlying reason? Thoughts?
